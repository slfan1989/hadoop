...
public class ITestS3AInputStreamPerformance extends S3AScaleTestBase {
  ...
  private void assertOpenOperationCount(long expected) {
    assertEquals(
       expected, streamStatistics.getOpenOperations(), "open operations in\n" + in);
  }
  ...
  public void testRandomIORandomPolicy() throws Throwable {
    ...
    assertEquals(
       0, streamStatistics.getAborted(), "streams aborted in " + streamStatistics);
  }
  ...
  public void testRandomIONormalPolicy() throws Throwable {
    ...
    assertEquals(
       1, streamStatistics.getAborted(), "streams aborted in " + streamStatistics);
    assertEquals(
       2, streamStatistics.getPolicySetCount(), "policy changes in " + streamStatistics);
    assertEquals(
...
,         streamStatistics.getInputPolicy(), "input policy in " + streamStatistics);
  ...
  public void testRandomReadOverBuffer() throws Throwable {
    ...
    assertEquals(
       readahead - 1, s3aStream.remainingInCurrentRequest(), "remaining in\n" + in);
    assertEquals(
       0, s3aStream.getContentRangeStart(), "range start in\n" + in);
    assertEquals(
       readahead, s3aStream.getContentRangeFinish(), "range finish in\n" + in);
    ...
    while (bytesRead < halfReadahead) {
      ...
      readOps++;
      assertEquals(
         1, streamStatistics.getOpenOperations(), "open operations on request #" + readOps
    ...
    assertEquals(sourceData[currentPos]
,         (int) buffer[currentPos], "Wrong value from read ");
    currentPos++;
    ...
    while(currentPos < datasetLen) {
      int r = in.read();
      assertTrue(
         r >= 0, "Negative read() at position " + currentPos + " in\n" + in);
      buffer[currentPos] = (byte)r;
      assertEquals(
         sourceData[currentPos], r, "Wrong value from read from\n" + in);
      currentPos++;
      readCount++;
    }
